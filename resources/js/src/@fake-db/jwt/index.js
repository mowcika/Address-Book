...
mock.onPost('/jwt/handleComplete')
    .reply(request => {
        var user_data = JSON.parse(request.data)
        try {
            const accessToken = jwt.sign({ id: user_data.id }, jwtConfig.secret, { expiresIn: jwtConfig.expireTime })
            const refreshToken = jwt.sign({ id: user_data.id }, jwtConfig.refreshTokenSecret, {
                expiresIn: jwtConfig.refreshTokenExpireTime,
            })

            console.log('777')
            console.log(user_data)
            var usergroup = user_data.return_data.usergroup
            console.log('888')
            user_data.return_data.avatar = '/images/_/_/_/_/jobspro/resources/js/src/assets/images/avatars/13-small.png'
            user_data.return_data.role = usergroup
            user_data.return_data.ability = [
                {
                    'action': 'manage',
                    'subject': 'all'
                }
            ]
            user_data.return_data.extras = {
                'eCommerceCartItemsCount':
                    5
            }

            const userData = { ...user_data.return_data }
