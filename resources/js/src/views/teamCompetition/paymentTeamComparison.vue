<template>
    <b-card-code title="TEAM-WISE CHART COMPARISON">
...
export default {
    ...
    async created() {
        ...
        // job source
        const callAxios_js = await this.callAxios('/getjobSourceData', {}, 'post')
        this.jobSourceArray = callAxios_js.data.jobsourceDrop
        this.jobSourceArray.unshift({
    ...
    methods: {
        ...
        async timeOutHttp() {
            ...
            if (callAxios.data.getData.length > 0) {
                ...
                this.sweetAlertmethod('warning', 'No Data', '', 'btn-danger')
                this.initial_loading = true
            }
        ...
        async dynamicChartSection(chartType, labelArray, amountArray) {
            ...
            let thisArrayMax = parseInt(Math.max.apply(null, amountArray))
            // var getMinLength = thisArrayMin.toString().length;
            ...
            // console.log("getSumOfArray test : "+getSumOfArray)
            ...
            this.$refs.realtimeChart.updateOptions({
                title: {
                    text: this.userGroupChartTitle + '[ Total ' + getSumOfArray + '   -    ' + 'Teams ' + amountArray.length + ' ]'
                }
            })
