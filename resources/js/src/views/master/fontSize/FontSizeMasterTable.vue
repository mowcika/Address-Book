<template>
    <b-card-code title="Add Font Size">
        <!-- input search -->
        ...
        >
          ...
          <span>
            ...
            >
              ...
              <b-dropdown-item @click="callDeleteMethod('/deleteFontMaster', data={id:props.row.id}, 'post')">
                <feather-icon
                    class="mr-50"
                    icon="TrashIcon"
                />
        ...
        >
            <validation-observer ref="simpleRules">
                <b-form>
                    ...
                    <b-form-group
                        label="Find Size Name"
                        label-for="Find Size Name"
                    >
                        <validation-provider
                            #default="{ errors }"
                            name="Find Size Name"
                        ...
                        >
                            <b-form-input
                                v-model="data.fontName"
                                :state="errors.length > 0 ? false:null"
                                placeholder="Find Size Name"
                            />
                            <small class="text-danger">{{ errors[0] }}</small>
                        </validation-provider>
                    </b-form-group>
                    <b-form-group
                        label="Find Size Value"
                        label-for="Find Size Value"
                    >
                        <validation-provider
                            #default="{ errors }"
...
export default {
    ...
    methods: {
        async showModal(value) {
            if (value) {
                this.data.id = value
                const callAxios = await this.callAxios('/getSingleFontMaster', { id: value }, 'post')
                console.log(callAxios)
                this.data.fontSize = callAxios.data[0]['font_size']
                this.data.fontName = callAxios.data[0]['font_name']
            ...
            this.$refs.simpleRules.validate()
                .then(async success => {
                    if (success) {
                        ...
                        const callAxios = await this.callAxios('/saveFontName', this.data, 'post')
