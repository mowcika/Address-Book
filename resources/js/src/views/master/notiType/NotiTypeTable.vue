<template>
    <b-card-code title="Notification Type">
        <!-- input search -->
        ...
        >
          ...
          <span>
            ...
            >
              ...
              <b-dropdown-item @click="callDeleteMethod('/deletenotiType', data={id:props.row.id}, 'post')">
                <feather-icon
                    class="mr-50"
                    icon="TrashIcon"
                />
...
export default {
    ...
    methods: {
        async showModal(value) {
            if (value) {
                this.data.id = value
                const callAxios = await this.callAxios('/getSinglenotiType', { id: value }, 'post')
                console.log(callAxios)
                this.data.notiType = callAxios.data[0]['noti']
                this.data.notiValue = callAxios.data[0]['noti_value']
            ...
            this.$refs.simpleRules.validate()
                .then(async success => {
                    if (success) {
                        ...
                        const callAxios = await this.callAxios('/savenotiType', this.data, 'post')
