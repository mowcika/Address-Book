<template>
    <b-card-code title="Language Master">
        <!-- input search -->
        ...
        >
          ...
          <span>
            ...
            >
              ...
              <b-dropdown-item @click="callDeleteMethod('/deleteLang', data={id:props.row.id,user:data.inby}, 'post')">
                <feather-icon
                    class="mr-50"
                    icon="TrashIcon"
                />
...
export default {
    ...
    methods: {
        async showModal(value) {
            if (value) {
                this.data.id = value
                const callAxios = await this.callAxios('/getSingleLang', { id: value }, 'post')
                console.log(callAxios)
                this.data.lang = callAxios.data[0]['lang']
                this.data.logo = callAxios.data[0]['logo']
                this.data.inby = userData.id
            ...
            this.$refs.simpleRules.validate()
                .then(async success => {
                    if (success) {
                        ...
                        const callAxios = await this.callAxios('/saveLang', this.data, 'post')
